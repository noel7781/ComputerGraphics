...
def exp(rv):
    ...
    u = normalized(rv)
    rm = np.identity(3)
    rm[0, 0] = np.cos(th) + u[0] * u[0] * (1 - np.cos(th))
    rm[0, 1] = u[0] * u[1] * (1 - np.cos(th)) - u[2] * np.sin(th)
    rm[0, 2] = u[0] * u[2] * (1 - np.cos(th)) + u[1] * np.sin(th)
    rm[1, 0] = u[1] * u[0] * (1 - np.cos(th)) + u[2] * np.sin(th)
    rm[1, 1] = np.cos(th) + u[1] * u[1] * (1 - np.cos(th))
    rm[1, 2] = u[1] * u[2] * (1 - np.cos(th)) - u[0] * np.sin(th)
    rm[2, 0] = u[2] * u[0] * (1 - np.cos(th)) - u[1] * np.sin(th)
    rm[2, 1] = u[2] * u[1] * (1 - np.cos(th)) + u[0] * np.sin(th)
    rm[2, 2] = np.cos(th) + u[2] * u[2] * (1 - np.cos(th))
    #print (np.linalg.det(rm))
    return rm

def log(rm):
    th = np.arccos((rm[0, 0] + rm[1, 1] + rm[2, 2] - 1) / 2)
    rv = np.array([.0, .0, .0])
    rv[0] = (rm[2, 1] - rm[1, 2]) / (2 * np.sin(th))
    rv[1] = (rm[0, 2] - rm[2, 0]) / (2 * np.sin(th))
    rv[2] = (rm[1, 0] - rm[0, 1]) / (2 * np.sin(th))
    return rv * th
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2016025996-10-1', None,None)
    if not window:
        glfw.terminate()
        return
