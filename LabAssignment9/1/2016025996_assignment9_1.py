...
def render():
    global gCamAng, gCamHeight, gAlpha, gBeta, gGamma
    ...
    # ZXZ Euler angles
    t = glfw.get_time()
    alpha = np.radians(gAlpha)
    beta = np.radians(gBeta)
    gamma = np.radians(gGamma)
    M = np.identity(4)
    M[:3,:3] = RotateZ(alpha) @ RotateX(beta) @ RotateZ(gamma)
    glMultMatrixf(M.T)
